# ⚙️ 04: High-Performance Piston Head CAD Design

## Project Overview

This is a detailed CAD model of a **High-Performance Piston Head**, specifically focusing on the complex geometry and critical features of the crown, ring areas, and wrist pin connection. The project showcases expertise in mechanical component modeling and technical documentation for high-stress, internal combustion engine applications.

## Design Focus & Key Features

* **Component:** Piston Head (Excluding Connecting Rod)
* **Skill Highlight:** Emphasis on creating precision-modeled functional elements essential for sealing, lubrication, and combustion within the cylinder bore.
* **Modeled Components & Features:**
    * **Crown:** The top surface, including the detailed **Combustion Area** and **Valve Recess** geometry.
    * **Ring Lands** and **Ring Gap**.
    * **Compression Ring Grooves** and **Oil Ring Groove**.
    * **Oil Drain Holes** within the oil ring groove for lubrication management.
    * **Skirt** geometry for cylinder stabilization.
    * **Wrist Pin Boss** (for gudgeon pin connection).
* **Documentation:** Project includes a professional **2D dimensional drawing** (`Drawing.jpg`) essential for manufacturing and quality control.

## Deliverables

| File | Description | Link |
| :--- | :--- | :--- |
| **Main Render** (`Piston.jpg`) | Primary view of the complete Piston Head component. | (https://github.com/tchara96/CAD-projects/blob/main/04_High_Performance_Piston/Piston_orthographic.jpg) |
| **Drawing** (`Drawing.jpg`) | 2D dimensional drawing with orthographic views and manufacturing specifications. | [Link to Drawing.jpg] |
| **Detailed Renders** | Views showing the top (crown), bottom (pin boss), and sides. | [Link to Piston top.jpg], [Link to Piston bottom.jpg], [Link to Piston Side.jpg], [Link to Piston Side 2.jpg] |
| **CAD Source File** | Placeholder for the primary CAD file (e.g., `.step`, `.ipt`). | [Link to CAD File] |

## Key Skills Demonstrated

* **Mechanical Component Design:** Precision modeling of critical, high-tolerance engine components.
* **Detailed Feature Modeling:** Accurate representation of functional elements (ring grooves, valve recesses, oil drains).
* **Technical Drafting:** Creating professional **2D drawings**, including dimensioning and tolerances.
